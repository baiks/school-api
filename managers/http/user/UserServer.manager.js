...
module.exports = class UserServer {
    ...
    _setup() {
        ...
        // ── Health ────────────────────────────────────────────────────────────
        app.get('/health', (req, res) => res.json({ ok: true, service: config.dotEnv.SERVICE_NAME }));
        ...
        app.post('/api/auth/register',
            ...
            async (req, res) => {
                ...
                const result = await userManager.register({ username, email, password, role, schoolId });
        ...
        app.post('/api/auth/login',
            ...
            async (req, res) => {
                const result = await userManager.login({ email: req.body.email, password: req.body.password });
                return result.error ? err(res, result) : ok(res, result);
            }
        );
        ...
        app.post('/api/auth/refresh',
            validate(['longToken']),
            async (req, res) => {
                const result = await userManager.refreshToken({ longToken: req.body.longToken });
                return result.error ? err(res, result) : ok(res, result);
            }
        );

        app.get('/api/auth/me', anyAdmin, async (req, res) => {
            const result = await userManager.getById(req.user.userId);
            return result.error ? err(res, result) : ok(res, result);
        });

        // ── User Management Routes (superadmin only) ──────────────────────────
        app.get('/api/users', superOnly, async (req, res) => {
            const result = await userManager.list(req.query);
            return result.error ? err(res, result) : ok(res, result);
        });

        app.delete('/api/users/:id', superOnly, async (req, res) => {
            const result = await userManager.deactivate(req.params.id);
            return result.error ? err(res, result) : ok(res, result);
        });

        // ── Entity Sub-Routers ────────────────────────────────────────────────
        // Each entity manages its own routes; auth middleware applied here before mounting
        ...
        // ── 404 ───────────────────────────────────────────────────────────────
        app.use((req, res) => {
            res.status(404).json({ ok: false, message: `Route ${req.method} ${req.path} not found` });
        });
    ...
    run() {
        ...
        this.app.listen(port, () => {
            console.log(`[${this.config.dotEnv.SERVICE_NAME}] Server running on port ${port}`);
